...
if (isset($_POST['registrarUser'])) {
    ...
    if (empty($userName) || empty($nombre) || empty($apellidos) || empty($dni) || empty($edad) || empty($email) || empty($pass) || empty($pass2)) {
        array_push($errores, "<div class='row alert alert-danger'>*Es obligatorio completar todos los campos</div>");
    } else {
        if ($pass != $pass2) {
...
<div class="card border-danger rounded-0">
    <div class="card-header p-0">
        <div class="bg-dark text-white text-center py-2">
            <h2 style="padding-top: 1em;">Registrarse usuario</h2>
            <br>
        </div>
    </div>
        ...
        <form id="formRegister" onkeyup="validarFormulario(event)" method="POST" action="<?php echo $_SERVER["PHP_SELF"] ?>?p=registerUser" enctype="multipart/form-data">
            <div class="form-row">
                ...
                <div class="form-group col-md-4">
                    ...
                    <input value="" name="edad" type="text" class="form-control" id="edad">
                </div>
                <br>
            </div>
            <div class="form-row">
